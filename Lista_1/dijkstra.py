...
def dijkstra(graph_dict: dict[str, Stop], start_stop, start_time: int):
    ...
    times[start_stop] = 0
    pq = [(0, start_stop)]
    prev_nodes = {node: (None, None) for node in graph_dict}
    ...
    while pq:
        ...
        for connection in graph_dict[curr_node].connections:
            new_time = curr_time + calculate_time(curr_time + start_time, connection)

            if new_time < times[connection.end_stop.name]:
                times[connection.end_stop.name] = new_time
                prev_nodes[connection.end_stop.name] = (curr_node, connection)
                heapq.heappush(pq, (new_time, connection.end_stop.name))
